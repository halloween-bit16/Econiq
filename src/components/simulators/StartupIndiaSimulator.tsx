...
const StartupIndiaSimulator = () => {
  ...
  const comparisonData = [
    { name: "Without 80-IAC", profit: annualProfit - calculations.normalTax, fill: "hsl(220, 15%, 30%)" },
    { name: "With 80-IAC", profit: calculations.netProfit, fill: "hsl(142, 76%, 45%)" },
  ];
    ...
    <div className="grid gap-6 lg:grid-cols-2">
      ...
      <div className="bg-card border border-border rounded-xl p-6 space-y-6">
        ...
        <div className="space-y-3">
          ...
          <div className="relative h-3 bg-muted rounded-full overflow-hidden flex">
            <div className="h-full bg-primary/40" style={{ width: `${(3 / 12) * 100}%` }} />
            <div className="h-full bg-secondary/30" style={{ width: `${(7 / 12) * 100}%` }} />
            <div 
              className="absolute top-0 bottom-0 w-0.5 bg-destructive"
              style={{ left: `${(10 / 12) * 100}%` }}
            />
          </div>
        ...
        <div className="space-y-3">
          ...
          <Slider
            value={[hasUsedHoliday]}
            onValueChange={(v) => setHasUsedHoliday(v[0])}
            min={0}
            max={3}
            step={1}
          />
        ...
        <div className={`flex items-center justify-between p-4 rounded-lg border ${
          isDPIITRecognized ? 'border-primary/30 bg-primary/5' : 'border-border bg-muted/30'
        }`}>
          <div>
          ...
          <Switch
            checked={isDPIITRecognized}
            onCheckedChange={setIsDPIITRecognized}
          />
        </div>
      </div>
      ...
      <div className="space-y-6">
        ...
        <div className="bg-muted/30 border border-border rounded-lg p-4">
          ...
          <p className="text-sm text-muted-foreground">
            {calculations.canClaimHoliday ? (
              <>Without Section 80-IAC, your profit of <span className="text-foreground font-medium">{formatCurrency(annualProfit)}</span> would 
              ...
              With the exemption, you pay <span className="text-primary font-medium">₹0</span> and keep the full profit.</>
            ) : (
              <>Your profit of <span className="text-foreground font-medium">{formatCurrency(annualProfit)}</span> falls 
            ...
            {eligibilityCriteria.map((item, index) => (
              <div 
                ...
                className={`flex items-start gap-2 p-3 rounded-lg border ${
                  item.met ? 'bg-primary/5 border-primary/20' : 'bg-destructive/5 border-destructive/20'
                }`}
              >
                {item.met ? (
